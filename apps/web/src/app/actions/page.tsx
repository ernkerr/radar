// Actions page -- displays recommended actions generated by the analysis pipeline
// in response to alerts. Actions represent concrete steps the user should take
// (e.g. update ERP records, contact a supplier, place an inventory hold).
// Each action goes through an approval workflow: pending -> approved/rejected ->
...
// Type definition matching the actions table. The `alerts` field comes from
// a FK join and provides context about which alert triggered this action.
type Action = {
  id: string;
  // Action type taxonomy -- four categories of recommended actions:
  // - "task": a manual review task for the compliance team
  // - "erp_update": a suggested change to ERP/inventory system records
  // - "outreach": a recommendation to contact a supplier for updated docs
  // - "hold": a recommendation to place inventory on hold pending resolution
  ...
  created_at: string;
  // Joined from the alerts table via the alert_id FK -- provides the originating
  // alert's title and urgency for display context.
  alerts: { title: string; urgency: string } | null;
};

// Maps each action type to its icon component for visual differentiation.
...
// Human-readable labels for each action type shown in the UI badge.
...
export default function ActionsPage() {
  ...
  // Default filter is "pending" so users immediately see actions needing attention.
  ...
  // Approve/reject perform two writes each:
  // 1. Update the action's status in the actions table (drives the workflow state).
  // 2. Insert a record into the approvals table to create an audit trail.
  //    The approval record is separate because it tracks WHO approved/rejected
  //    and WHEN, which is important for compliance auditing. The actions table
  //    only stores the current status, not the history of decisions.
  ...
  // Client-side status filtering. "all" shows everything, otherwise match exactly.
